...
| `global_excludes` | list | 空 | **全局排除项**：所有会话无条件生效，写在这里就不用每个群再设一遍。可填预设名（`简体` / `720p` / `Baha`…）或任意关键词。 |
...
内置预设 16 组，写预设名会自动展开成一串同义写法，省得逐条手打：

| 预设 | 展开成 |
| --- | --- |
| `简体` | 简体、简日、CHS、GB |
| `繁体` | 繁体、繁日、CHT、BIG5 |
| `简繁` | 简繁、繁简、CHS&CHT、GB&BIG5 等双语包写法 |
| `720p` / `1080p` / `2160p` | 含对应分辨率的两种写法（如 `1080p` 与 `1920x1080`） |
| `Baha` / `ABEMA` / `CR` / `B-Global` | 对应片源标识（Bahamut、Crunchyroll、Bilibili 等） |
| `内嵌` / `外挂` | hardsub / softsub、内封 |
| `MKV` / `MP4` | 容器格式 |
| `合集` | 合集、Batch、BDRip |
| `生肉` | 生肉、无字幕、Raw |
