...
| `/sub_exclude [list|add <词>|del <词>|clear|preset|apply]` | 全局排除项：标题命中的发布直接丢掉。新订阅自动套用，`apply` 可刷到已有订阅。 | `/排除词` |
...
共 54 项，全部可以在 Dashboard 的插件配置页或本插件自带的管理页里改。**开箱默认值就能用，下面这些都是可选的调整。**
...
| **配置** | 54 项配置按 10 组编辑，开关/滑块/下拉/多行文本各按类型给控件，带说明与默认值提示。密钥类字段只显示「已配置」不回显 |
| **追番** | 选一个会话，改进度、改状态、打分、写备注、删条目；也能直接在这里搜名字加进追番表 |
| **订阅** | 增删订阅、单条测试抓取、批量启停、导出 JSON / 粘贴导入；还能**只填番名点「列字幕组」**从 Mikan 候选里挑一个订上，以及勾选**全局排除项**并一键回写到已有订阅 |
...
| **配置** · 54 项分 10 组，控件按类型自动匹配 | **追番** · 进度 / 状态 / 评分 / 备注就地改 |
...
选了单组之后还是会有重复（同一组把简繁各发一遍）。`/sub_exclude` 给当前会话管一份**对本会话所有订阅生效**的排除词表，
标题命中任意一个词的发布直接丢掉，不推、不入库：
...
/sub_exclude apply        把当前词表刷到已有订阅上
/sub_exclude clear        清空
```

内置预设 12 组，写预设名会自动展开成一串同义词，省得逐条手打：

| 预设 | 展开成 |
| --- | --- |
| `简体` | 简体、简日、CHS、GB |
| `繁体` | 繁体、繁日、CHT、BIG5 |
| `720p` / `1080p` / `2160p` | 含对应分辨率写法（如 1920x1080） |
| `合集` | 合集、Batch、BDRip |
| `生肉` | 生肉、无字幕、Raw |
| `Baha` / `ABEMA` / `CR` / `B-Global` / `MP4` | 对应片源或容器标识 |

排除词**新加的订阅自动套用**；已经存在的订阅不会被偷偷改写，需要显式 `apply`（或在管理页点「回写到已有订阅」）。
...
如果是同一个组把简繁 / 画质各发一遍，用 `/sub_exclude preset` 勾掉不想要的，再 `/sub_exclude apply` 刷到已有订阅上。
...
│   └── web/                管理页后端（32 条路由）与独立 Webhook 监听
