...
- **订阅先选组，不刷屏。** `/sub 番名` 会先列出 Mikan 上的字幕组，回序号才订阅，订上的是单组 RSS ——
  ...
  和**同集归并** —— 同一集的简繁 / 画质 / Baha 与 ABEMA 多个版本，只挑最合你口味的那一条推。
...
共 57 项，全部可以在 Dashboard 的插件配置页或本插件自带的管理页里改。**开箱默认值就能用，下面这些都是可选的调整。**
...
| `global_excludes` | list | 空 | **全局排除项**：所有会话无条件生效，写在这里就不用每个群再设一遍。可填预设名（`合集` / `生肉` / `720p`…）或任意关键词。片源与语言口味建议交给下面的 `rss_episode_prefer`，[原因见这里](#全局层到底该填什么)。 |
...
排除片源能治表面，但代价不小：把 `ABEMA` 排掉之后，哪天 Baha 没出片，这一集你就整集收不到。
所以默认做法是**都收下，只推一条**：
...
两个手段分工不同，填错层会白白少收消息。建议这样分：

| 放全局排除项 | 交给 `rss_episode_prefer` |
| --- | --- |
| 任何形式都不想要的东西：`合集`、`生肉`，介意流量的话再加 `720p` | 语言、画质、片源的**口味**：`简体`、`1080p`、`Baha`… |
| 命中即丢弃，一条都不会来 | 都收下，同一集里只挑最合口味的那条推 |

具体说，**不建议**把 `ABEMA` / `CR` / `B-Global` 这类片源放进全局层：同集归并已经保证「同组同集只推一条」，
硬屏蔽片源反而会让 Baha 没出片的那天整集收不到；而 `B-Global` 展开后含 `Bilibili`，会连带打死
`[Up to 21°C]`、`[NC-Raws]` 这些**简日双语**发布，损失比收益大得多。

只有一种情况该在全局层写语言和片源：订的是**宽 feed**（关键词搜索、`dmhy:` 全站源这类一条 feed 里混着多个组）。
归并不跨组，这时排除项是唯一的收敛手段。
...
如果是同一个组把简繁 / 画质各发一遍，先确认配置项 `rss_episode_dedup` 是开着的（默认开，同一集只留一条），
再用 `/sub_exclude preset` 勾掉彻底不想看到的版本，或在配置项 `global_excludes` 里一次设好全部会话。
